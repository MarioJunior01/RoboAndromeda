...
from pybricks.parameters import Color, Button, Direction
...
class MuxColorSensor(Ev3devMuxSensor):
...
class MuxInfraredSensor(Ev3devMuxSensor):
...
class MuxGyroSensor(Ev3devMuxUartSensor):
    ...
    def __init__(self, port, mux, direction=Direction.CLOCKWISE):
        Ev3devMuxUartSensor.__init__(self, port, mux)
...
class MuxUltrasonicSensor(Ev3devMuxSensor):
